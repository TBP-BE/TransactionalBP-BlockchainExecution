    ...
    return new Promise((resolve, reject) => {
        parseBpmn(modelInfo.bpmn)
            .then((definitions: any) => {
                modelInfo.solidity = "pragma solidity ^0.4.18;\n";
                ...
                for (let controlFlowInfo of globalControlFlowInfo) {
                    if (!controlFlowInfo.isEmbedded) {
                        ...
                        let codeGenerationInfo = {
                            ...
                            nodeImaginaryIndex : nodeId => {
                                //numbernode
                              var numbernode = 0;
                                controlFlowInfo.nodeList.forEach(function () {
                                        numbernode++;
                                ...
                                if (node.script) {
                                    ...
                                    var lines = node.documentation[1].text.split(";") ;
                                    var property = lines[0].split("=") ;
                                    var prop = property[1];
                                }
                            ...
                            nodeParameters: nodeId => {
                                ...
                                if (node.documentation && node.documentation[0].text && node.documentation[0].text.length > 0) {
                                    ...
                                    return resDict.get("input").length > 0 || resDict.get("output").length > 0;
                                }
                                return false;
                            },
                            ...
                            nodeFunctionBody: nodeId => {
                                ...
                                if (node.script) {
                                    ...
                                    var pos = node.documentation[0].text.indexOf(";");
                                    //var pos1 = node.documentation[0].text.indexOf("}");
                                    var pos1 = node.documentation[0].text.indexOf("}");
                                var property = node.documentation[0].text.slice(pos +1 , pos1);
                                return extractParameters(property, nodeId, null).get("body");

                                    //var res1 = property.substring(0, property.length - 1);
                                   //return property;                               
                                } 
                                else 
                                return "";
                            ...
                            nodeTransactionalProperty: nodeId => {
                                let node = globalNodeMap.get(nodeId);
                                if (node.script) {
                                    console.log("node script");
                                    return node.script.split("->");
